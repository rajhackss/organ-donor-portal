...
export default function RecipientDashboard() {
    ...
    const [organRequired, setOrganRequired] = useState('');
        ...
        async function fetchProfile() {
            if (currentUser) {
                ...
                if (docSnap.exists()) {
                    ...
                    setOrganRequired(data.organRequired || '');
                    setUrgencyLevel(data.urgencyLevel || 'Low');
                }
                setLoading(false);
            }
        }
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                const data = doc.data();
                // Filter verified and active donors in memory to avoid needing
                // a composite index for multiple where() clauses in Firestore.
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex-grow max-w-7xl w-full mx-auto py-10 px-4 sm:px-6 lg:px-8">
                ...
                <p className="mt-2 text-gray-600">Manage your organ requests and view statuses below.</p>
                            ...
                            <div className="px-4 py-5 sm:p-6">
                                ...
                                <form onSubmit={handleUpdateProfile} className="mt-6 space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                                        ...
                                        <div className="sm:col-span-3">
                                            <label htmlFor="organRequired" className="block text-sm font-medium text-gray-700">
                                                Organ Required
                                            </label>
                                            <select
                                                id="organRequired"
                                        ...
                                        <div className="sm:col-span-6">
                                            ...
                                            >
                                                ...
                                                <option value="Medium">Medium - Standard Waiting List</option>
                                                <option value="High">High - Critical Care Required</option>
                                        ...
                                        <div className="sm:col-span-6 mt-2">
                                            ...
                                            <textarea
                                                ...
                                                placeholder="Please describe any pre-existing medical conditions or reasons for needing the organ..."
                                    ...
                                    <div className="pt-4 flex justify-end">
                                        ...
                                        >
                                            {saving ? 'Saving...' : 'Update Medical Request'}
                                        </button>
                                    </div>
                                </form>
                            ...
                            <div className="bg-white shadow overflow-hidden sm:rounded-lg mt-8">
                                <div className="px-4 py-5 sm:p-6">
                                    ...
                                    {availableDonors.length === 0 ? (
                                        <p className="text-sm text-gray-500">Currently, there are no verified donors in the system.</p>
                                    ) : (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {availableDonors.map((donor) => {
                                                const matchesOrgan = organRequired && donor.organAvailable === organRequired;
                                                const matchesBlood = bloodGroup && donor.bloodGroup === bloodGroup;

                                                return (
                                                    <div key={donor.id} className={`border rounded-lg p-4 ${matchesOrgan ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-white'}`}>
                                                        <div className="flex justify-between items-start">
                                                            <div>
                                                                ...
                                                                <p className="text-sm text-gray-500 mt-1">Age: {donor.age || 'N/A'}</p>
                                                            </div>
                                                            <div className="text-right">
                                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${matchesBlood ? 'bg-rose-100 text-rose-800' : 'bg-gray-100 text-gray-800'}`}>
                                                                    Blood: {donor.bloodGroup || 'Unknown'}
                                                                </span>
                                                                <p className="text-sm font-medium text-gray-900 mt-1">Offering: {donor.organAvailable || 'Unknown'}</p>
                                                            </div>
                                                        </div>
                                ...
                                <div className="flex">
                                    <div className="ml-3">
                                        <p className="text-sm text-yellow-700">
                                            Your account must be <b>Verified</b> by an administrator before you can view and contact available organ donors. Current Status: {profile?.status || 'Pending'}
                                        </p>
                                    </div>
                                </div>
                            </div>
